...
function start() {
  ...
  for(let result of json){
    ...
    a.appendChild(div)
    a.appendChild(htmlToNode(`<p class="search-info">From ${result.foundIn} (${result.page[result.foundIn].toString().replaceAll(new RegExp(term, "gi"), x => "<span class='found'>"+x+"</span>")})</p>`))
  ...
  if(json.length === 0){
    ...
    let a = createLinkForResult(result)
    a.appendChild(div)
    a.appendChild(htmlToNode(`<p class="search-info no-results">Search '${term}' had no matches</p>`))
    document.getElementById("results").appendChild(a)
  }
  document.getElementById("query").innerText = "Page search for: '"+term+"'"
